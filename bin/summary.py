...
def calculate_cluster_stats(input_file, output_file):
    ...
    and count of clusters with max, mean, and median > 10,000 for specified columns
    in the input CSV file.
    Filter out clusters with a max value below 100.
    Also, count clusters with ALL specified samples above 100 and 10000.
    ...
    # Ensure that all selected columns exist in the DataFrame
    for col in selected_columns:
        if col not in df.columns:
            raise KeyError(f"The column '{col}' is not found in the input file.")

    # Filter the DataFrame to only keep the selected columns
    df = df[['cluster'] + selected_columns]

    # Set 'cluster' as index for calculations
    df = df.set_index('cluster')

    # Calculate max, mean, median for each cluster
    stats_df = df.agg(['max', 'mean', 'median'], axis=1)
    ...
    clusters_above_100 = {
        ...
        'all_samples_above_100': ((df > 100).all(axis=1)).sum()
    }
    ...
    clusters_above_10000 = {
        ...
        'all_samples_above_10000': ((df > 10000).all(axis=1)).sum()
    }

    # Print the counts
    print(f"{ {output_file} }100:{clusters_above_100}")
    print(f"{ {output_file} }10000:{clusters_above_10000}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Calculate max, mean, median, and count clusters with values > 100 and > 10,000 for specified columns in a CSV file. Filter out clusters with a max value below 100. Also, count clusters with ALL specified samples above 100 and 10000.")
